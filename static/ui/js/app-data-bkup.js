...
class ApplicationDataManager {
	...
	async tryRecentFilesFallback() {
		...
		// Priority order for file patterns (most specific to least specific)ff
		const filePatterns = [
	...
	async discoverFilesWithPattern(basePath, pattern) {
		...
		for (const timestamp of commonTimestamps) {
			for (const prefix of prefixes) {  // This loop was missing in the original
				const testPaths = [
    ...
    async loadApplications() {
        console.log('Extracting applications from traffic data...');
	...
	async loadTrafficData() {
		...
		if (!this.config.csvPaths.traffic) {
			console.warn('No traffic CSV path available');
			return;
		}
		...
		try {
			console.log('Attempting to load traffic from:', this.config.csvPaths.traffic);
			const response = await fetch(this.config.csvPaths.traffic);
			if (!response.ok) {
				console.warn(`Traffic data not available: HTTP ${response.status} for ${this.config.csvPaths.traffic}`);
			...
			console.log(`Traffic CSV loaded: ${csvText.length} characters from ${this.config.csvPaths.traffic}`);
	...
	async tryAlternativeTrafficPaths() {
		...
		// Use the same generic discovery logic instead of hardcoded paths
		return await this.tryRecentFilesFallback();
	}
	...
	enrichApplicationsWithTraffic() {
		...
		appTrafficMap.forEach((trafficData, appName) => {
			if (existingApps.has(appName)) {
				// Update existing application with traffic data
				const existingApp = existingApps.get(appName);
				console.log(`Updating existing app: ${appName}`);
				...
				// Create new application from traffic data
				console.log(`Creating new app from traffic: ${appName}`);
		...
		this.applications.forEach(app => {
			const hasTraffic = appTrafficMap.has(app.name);
			console.log(`  - ${app.name}: ${app.trafficRecords || 0} traffic records ${hasTraffic ? '(updated)' : '(preserved)'}`);
		});
	}
    
    // Add these debug methods to your ApplicationDataManager class
	...
	// Debug utilities to add to window.AppDataUtils
    ...
    async refreshData() {
        ...
        // Note: Don't clear applications array as applicationList.csv is static
        // and should be reloaded from the same location
	...
	// Add this method to handle CSV uploads
	...
	generateBasicTopology() {
		// This is just an alias for the main topology generation
    ...
    async switchToSpecificFile(filename) {
        console.log(`Switching to specific traffic file: ${filename}`);
        
        // Don't allow switching applicationList.csv as it's static
        if (filename === 'applicationList.csv') {
            console.warn('Cannot switch applicationList.csv - it is a static reference file');
            this.errors.push('applicationList.csv is a static file and cannot be switched');
            return this.getStatus();
        }
        
        // Set traffic CSV path (not applications)
        this.config.csvPaths.traffic = `/data_staging/${filename}`;
        
        // Reload traffic data with new file
        this.trafficData = [];
        await this.loadTrafficData();
        ...
        console.log(`Switched to ${filename}, loaded ${this.trafficData.length} traffic records`);
        return this.getStatus();
    }
	...
	saveTopologyToFile() {
		...
		try {
			...
			console.log(`Network topology saved: ${filename}`);
			console.log(`File includes: ${this.applications.length} apps, ${this.networkTopology.nodes?.length || 0} nodes, ${this.duplicateStats.totalProcessed} processed records (${this.duplicateStats.duplicatesIgnored} duplicates removed)`);
			...
			console.error('Failed to save topology file:', error);
	...
	async loadSpecificTopology(filename) {
		try {
			...
			// Restore the applications array AND mark them for preservation
			if (topologyData.applications) {
				...
				console.log(`Restored ${this.applications.length} applications:`, 
					this.applications.map(app => app.name));
				console.log('Preserved applications:', Array.from(this.preservedApplications));
			}
			...
			// ... rest of your existing loadSpecificTopology code ...
			
			console.log(`Loaded topology from ${filename} with ${this.preservedApplications.size} preserved apps`);
			return true;
...
window.AppDataUtils = {
	...
	// Dynamic Preservation utilities
	 preservation: {
...
console.log('Features: Dynamic CSV discovery, pipeline health monitoring, fallback handling');
console.log('Debug: Use AppDataUtils.pipeline.status() to check pipeline or AppDataUtils.status() for overall status');
