...
class NetworkTopologyDashboard {
    ...
    updateNetworkData() {
        if (!window.AppData) {
            console.warn('AppData not available for network data update');
            this.networkData = this.generateFallbackNetworkData();
            return;
        }
        
        console.log(`🔄 Updating network data - Apps: ${this.selectedApps.join(', ')}, Upstream: ${this.showUpstream}, Downstream: ${this.showDownstream}`);
        ...
        console.log(`🌐 Network data updated: ${this.networkData.nodes.length} nodes, ${this.networkData.links.length} links`);
        
        if (this.networkData.nodes.length > 0) {
            const appIds = [...new Set(this.networkData.nodes.map(n => n.applicationId))];
            const internalLinks = this.networkData.links.filter(l => l.isInternal).length;
            const interAppLinks = this.networkData.links.filter(l => !l.isInternal).length;
            
            console.log(`📊 Applications in view: ${appIds.join(', ')}`);
            console.log(`🔗 Links breakdown: ${internalLinks} internal, ${interAppLinks} inter-app`);
            
            this.updateFilterStats();
        }
    }
    
    // FIXED: Fallback network data for testing
    ...
    // ================================================================================
    // CORE FUNCTIONALITY (Continued in next part...)
    // ================================================================================
    ...
    setupFilters() {
        ...
        const displayOptions = {
            ...
            'show-traffic': () => {
                this.displayOptions.showTraffic = document.getElementById('show-traffic')?.checked || false;
        ...
        Object.entries(displayOptions).forEach(([checkboxId, handler]) => {
            ...
            if (checkbox) {
                ...
                if (checkboxId === 'show-traffic') checkbox.checked = this.displayOptions.showTraffic;
    ...
    filterData() {
        ...
        const filteredNodes = this.networkData.nodes.filter(node => {
            ...
            if (node.applicationId === 'ACDA') {
                console.log(`ACDA Node: ${node.name} (tier: ${node.tier}) -> ${nodeTypeMatch ? 'SHOW' : 'HIDE'}`);
            }
	...
	// If you're generating network data, make sure to assign correct colors:
	updateNetworkDataColors() {
		if (!window.topologyDashboard || !window.topologyDashboard.networkData) {
			console.log('No topology data to update');
			return;
		}
    
		const nodes = window.topologyDashboard.networkData.nodes;
    
		nodes.forEach(node => {
			// Override existing colors with correct ones
			const nodeType = (node.tier || node.type || '').toLowerCase();
			const nodeName = (node.name || '').toLowerCase();
        
			if (nodeName.includes('data') || nodeName.includes('storage') || nodeType.includes('database')) {
				node.color = '#eab308'; // Yellow for databases
			} else if (nodeName.includes('gateway') || nodeType.includes('gateway')) {
				node.color = '#8b5cf6'; // Purple for gateways  
			} else if (nodeName.includes('engine') || nodeType.includes('core')) {
				node.color = '#ef4444'; // Red for core services
			} else if (nodeType.includes('web') || nodeName.includes('web')) {
				node.color = '#3b82f6'; // Blue for web servers
			}
		});
    
		console.log('✅ Node colors updated to match legend');
	}
    ...
    renderTrafficFlow() {
        ...
        const activeLinks = this.networkData.links.filter(d => d.status === 'active');
        const traffic = this.trafficGroup.selectAll("circle.traffic")
            .data(activeLinks, d => `${d.source.id || d.source}-${d.target.id || d.target}`);
        
        traffic.exit().remove();
        
        const trafficEnter = traffic.enter().append("circle")
            .attr("class", "traffic")
            .attr("r", 2)
            .attr("fill", "#fbbf24")
            .style("opacity", 0.8);
        
        this.trafficElements = traffic.merge(trafficEnter);
    }
    
    startTrafficAnimation() {
        const animateTraffic = () => {
            if (this.displayOptions.showTraffic && this.trafficElements) {
                this.trafficElements
                    ...
                    .attrTween("transform", function(d) {
                        const link = d;
                        return function(t) {
                            if (!link.source || !link.target || !link.source.x || !link.target.x) return "";
            ...
            setTimeout(animateTraffic, 2500);
        };
    ...
    toggleTrafficFlow() {
        console.log(`Toggling traffic flow: ${this.displayOptions.showTraffic}`);
        this.renderTrafficFlow();
    }
...
// FOOLPROOF SOLUTION - Add this code to your topology.js file
// This approach doesn't rely on class definitions or complex dependencies

// ================================================================================
// SIMPLE PANEL TOGGLE SOLUTION - Add this anywhere in your topology.js
// ================================================================================
...
// === PANEL TOGGLE FIX - Add to END of topology.js ===

// Panel state tracking
window.panelStates = { left: false, right: false };
...
function togglePanelSimple(side) {
    ...
    const isCollapsed = window.panelStates[side];
    
    if (isCollapsed) {
        panel.classList.remove('collapsed');
        window.panelStates[side] = false;
        console.log(`📖 Expanded ${side} panel`);
    } else {
        panel.classList.add('collapsed');
        window.panelStates[side] = true;
        console.log(`📁 Collapsed ${side} panel`);
    }
    
    panel.offsetHeight; // Force reflow
    
    // Trigger resize events for canvas
    setTimeout(() => {
        window.dispatchEvent(new Event('resize'));
        if (window.topologyDashboard && window.topologyDashboard.handleResize) {
            window.topologyDashboard.handleResize();
...
// Fixed initialization with debouncing
function initPanelTogglesFixed() {
    console.log('🔧 Initializing panel toggles...');
    ...
    document.querySelectorAll('.hamburger').forEach(button => {
        ...
        button.addEventListener('click', function(e) {
            ...
            const panelSide = button.getAttribute('data-panel') || 
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initPanelTogglesFixed);
} else {
    initPanelTogglesFixed();
}
