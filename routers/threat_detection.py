...
class ThreatAlert(BaseModel):
    ...
    status: str = Field("active", pattern="^(active|investigating|resolved|false_positive)$")
...
class ResponseAction(BaseModel):
    ...
    action_type: str = Field(..., pattern="^(block_ip|isolate_host|alert_admin|quarantine_file|log_event)$")
...
async def get_alerts(
    ...
    status: Optional[str] = Query(None, pattern="^(active|investigating|resolved|false_positive)$"),
...
async def get_statistics(time_range: str = Query("24h", pattern="^(1h|6h|24h|7d|30d)$")):
